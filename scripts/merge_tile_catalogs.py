...
Usage:
  python scripts/merge_tile_catalogs_v2.py \
...
Optional (across all tiles with --write-master):
  ./data/tiles/_master_tile_catalog_pass2_raw.csv
  ./data/tiles/_master_tile_catalog_pass2.csv
"""
...
def merge_one_tile(tile_path: Path, tol_arcsec: float, overwrite: bool = False):
    catalogs_root = tile_path / "catalogs"
    files = [
        p for p in catalogs_root.glob("**/sextractor_pass2.csv")
    ...
    # Skip if outputs already exist and overwrite is not requested
    if not overwrite and raw_out.exists() and dedup_out.exists():
    ...
    for f in files:
        ...
        # If catalog is directly under catalogs/, use the tile name for image_id
    ...
    # Use the most common coord column names across frames
...
def main():
    ...
    ap.add_argument("--tiles-root", default="./data/tiles", help="Path to tiles root")
    ...
    ap.add_argument("--overwrite", action="store_true", help="Rewrite per-tile outputs even if they exist")
    args = ap.parse_args()
    ...
    for tile_path in tile_dirs:
        res = merge_one_tile(tile_path, args.tolerance_arcsec, overwrite=args.overwrite)
        if res == (None, None):
            continue
