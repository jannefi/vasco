...
# IMCCE conesearch endpoint (VO PHP)
...
def parse_args():
    ...
    # Behavior knobs (defaults tuned for real hits)
    p.add_argument("--field-radius-arcmin", type=float, default=22.0)        # cover 30' square footprint
    p.add_argument("--match-arcsec", type=float, default=5.0)                # strict
    p.add_argument("--fallback-wide-arcsec", type=float, default=60.0)       # MNRAS-like "1 arcmin" veto

    # Fallback per-row (OFF by default; enable from runner)
    p.add_argument("--fallback-per-row", type=str, default="false",
                   help="true/false (default=false). If true, try per-row 5\" cones where field returned 0 rows.")
    p.add_argument("--fallback-per-row-cap", type=int, default=100,
                   help="Safety cap: maximum rows per chunk for per-row cones (default=100).")

    # HTTP/runtime
    p.add_argument("--workers", type=int, default=1)  # sequential for clearer logs
...
def enrich(df: pd.DataFrame, t2p_path: Path, pep_path: Path, verbose=False) -> pd.DataFrame:
    ...
    # prefer ISO for tracing; keep JD numeric for the service
...
def grid_fields(df: pd.DataFrame, field_radius_arcmin: float) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Simple grid based on local angular step; yields ~100–200 rows/field at typical densities.
    """
    ...
    # Epoch per field: choose the MODE epoch across rows in that field
...
def parse_skybot_text_to_radec_list(text: str) -> List[Tuple[float,float]]:
    """
    Parse SkyBoT 'text' lines, extracting plausible (RAdeg,DEdeg) pairs without
    assuming fixed column positions. We look for adjacent floats with RA in [0,360]
    and Dec in [-90,90]. If multiple, we take the first per line.
    """
...
def call_skybot_field(ra_f: float, dec_f: float, epoch_iso: str, epoch_jd: float,
    ...
    """
    Field call using hyphenated params expected by the PHP conesearch.
    Radius supplied in arcminutes; converted to arcseconds for -rs.
    """
    rs_arcsec = rs_arcmin * 60.0
    params = {
        "-ra":     f"{float(ra_f):.8f}",
        "-dec":    f"{float(dec_f):.8f}",
        "-rs":     f"{float(rs_arcsec):.3f}",                # arcsec
        "-ep":     f"{float(epoch_jd):.6f}" if pd.notna(epoch_jd) else epoch_iso,
        "-mime":   "text",
        "-output": "all",
        "-refsys": "EQJ2000",
        "-equinox":"J2000",
    ...
    while True:
        try:
            if verbose:
                print(f"[HTTP] FIELD  -ra={params['-ra']} -dec={params['-dec']} -ep={params['-ep']} -rs={params['-rs']}", flush=True)
            r = requests.get(SKYBOT_URL, params=params, timeout=(ct, rt))
            status = r.status_code
            if status == 200:
                objs = parse_skybot_text_to_radec_list(r.text)
                return status, objs
...
def call_skybot_cone(ra: float, dec: float, epoch_iso: str, epoch_jd: float,
                     ...
                     verbose=False) -> Tuple[int, List[Tuple[float,float]]]:
    """
    Per-row cone using hyphenated params. Radius supplied in arcsec.
    """
    params = {
        "-ra":     f"{float(ra):.8f}",
        "-dec":    f"{float(dec):.8f}",
    ...
    while True:
        try:
            if verbose:
                print(f"[HTTP] ROW    -ra={params['-ra']} -dec={params['-dec']} -ep={params['-ep']} -rs={params['-rs']}", flush=True)
            r = requests.get(SKYBOT_URL, params=params, timeout=(ct, rt))
            status = r.status_code
            if status == 200:
                objs = parse_skybot_text_to_radec_list(r.text)
...
def process_chunk(args):
    ...
    # sequential for clarity/etiquette
    ...
    def angular_sep_arcsec(ra0_deg: float, de0_deg: float, ra1_deg: np.ndarray, de1_deg: np.ndarray) -> np.ndarray:
        # RA difference with wrap-safe normalization to [-180,180]
        dra = ((ra1_deg - ra0_deg + 180.0) % 360.0) - 180.0
        # small-angle approx is fine within 22' cones; use mean Dec for cosine factor
    ...
    for fid, sub in df.groupby("field_id"):
        ...
        if objs:
            objs_arr = np.array(objs, dtype=float)  # (N,2) with (RAdeg,DEdeg)
        ...
        if (fid in empty_fields) and fallback_per_row and (fb_attempted < args.fallback_per_row_cap):
            ...
            for _, r in sub_probe.iterrows():
                ...
                is_strict = (best_sep is not None) and (best_sep <= strict)
                is_wide = False  # per-row fallback only checks 5"
            ...
            # For remaining rows in the field (cap exhausted), emit unmatched
