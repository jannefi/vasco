...
Phase 1 (narrow): (tile_id, NUMBER, coords) -> dedupe grid -> join IR -> apply mask -> materialize survivor KEYS ONLY
...
CLI:
  ...
  --duckdb-mem <str>           "auto" or "14GB" etc. (default: "auto")
  --temp-dir <dir>             spill directory (default: /tmp/vasco_duckdb_tmp)
  --use-file-db                use file-backed DB near output

Notes:
  - At least one of --out or --out-dataset-dir must be provided.
...
def main():
    ...
    # Spill to SSD by default
    temp_dir = Path(a.temp_dir).resolve()
    temp_dir.mkdir(parents=True, exist_ok=True)

    # DB location
    db_path = (out_ds or (out_file.parent if out_file else Path.cwd())) / "export_tmp.duckdb" if a.use_file_db else Path(":memory:")
    db_path_str = db_path.as_posix()
    con = duckdb.connect(database=db_path_str)
    ...
    con.execute("PRAGMA memory_limit='auto';" if mem == "auto" else f"PRAGMA memory_limit={sql_quote(a.duckdb_mem)};")
    try:
    ...
    grid = float(a.dedupe_tol_arcsec) / 3600.0   # <-- fixed one-liner

    # Views:
    # Optical NARROW projection for Phase 1 (keys + coords + bins only)
    con.execute(f"""
    ...
    # Phase 1: dedupe on coarse sky grid, join IR, derive exclude_* columns, apply mask, KEEP KEYS ONLY.
    mask_expr = a.mask
    ...
    con.execute(f"""
        ...
        WHERE r.rn = 1
          AND (
                (NOT COALESCE(i.has_ir_match, FALSE))
                AND ({mask_expr})
          );
    """)
