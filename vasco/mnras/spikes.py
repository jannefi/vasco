...
def angsep_arcmin(ra1_deg: float, dec1_deg: float, ra2_deg: float, dec2_deg: float) -> float:
    ...
    s = 2*math.asin(math.sqrt(
        math.sin((dec2-dec1)/2)**2 + math.cos(dec1)*math.cos(dec2)*math.sin((ra2-ra1)/2)**2
    ))
...
def fetch_bright_ps1(ra_deg: float, dec_deg: float, radius_arcmin: float = 35.0,
                     rmag_max: float = 16.0, mindetections: int = 2) -> List[BrightStar]:
    """
    ...
    for row in rdr:
        try:
            ...
            if not (rmag <= rmag_max):
                continue
            ra = float(row['raMean']); dec = float(row['decMean'])
...
def apply_spike_cuts(tile_rows: Iterable[Dict[str,Any]], bright: List[BrightStar], cfg: SpikeConfig,
                     src_ra_key='ALPHA_J2000', src_dec_key='DELTA_J2000') -> Tuple[List[Dict[str,Any]], List[Dict[str,Any]]]:
    """
    ...
    for r in tile_rows:
        try:
            ...
            r2 = dict(r); r2['spike_reason'] = 'no_wcs'; kept.append(r2); continue
        # find nearest bright star
        dmin = 1e9; m_near = None
        for b in bright:
            d = angsep_arcmin(ra, dec, b.ra, b.dec)
            if d < dmin:
                dmin = d; m_near = b.rmag
        # default: keep if no bright star within search radius
        if not (dmin <= cfg.search_radius_arcmin and m_near is not None):
            r2 = dict(r); r2['spike_reason'] = '' ; kept.append(r2); continue
        # evaluate rules
        reject = False; reason = []
        for rule in cfg.rules:
            if isinstance(rule, SpikeRuleConst):
                if m_near <= rule.const_max_mag:
                    reject = True; reason.append(f'CONST(m*={m_near:.2f}<= {rule.const_max_mag:.2f})')
            elif isinstance(rule, SpikeRuleLine):
                thresh = rule.a*dmin + rule.b
                if m_near <= thresh:
                    reject = True; reason.append(f'LINE(m*={m_near:.2f}<= {rule.a:.3f}*{dmin:.2f}+{rule.b:.2f}={thresh:.2f})')
        r2 = dict(r)
        if reject:
            r2['spike_reason'] = ';'.join(reason); r2['spike_d_arcmin'] = round(dmin,3); r2['spike_m_near'] = m_near
            rejected.append(r2)
        else:
            r2['spike_reason'] = ''; r2['spike_d_arcmin'] = round(dmin,3); r2['spike_m_near'] = m_near
            kept.append(r2)
