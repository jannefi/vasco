...
- Append-only publishing is the default: existing master parts are not overwritten
  (preserves mtime_ns, so downstream incremental extractors won't see false deltas).
- --only-new: auto-skip tiles that already exist in the master dataset by filename.
- If --tiles-root points directly to one tile dir, process it.

Contract (unchanged):
- plate_id == DSS1-red FITS header REGION (e.g., "XE325") (mapping CSV provides 'irsa_region').
- IRSA PLATEID/PLTLABEL are intentionally NOT used for plate_id.

Examples:
  # publish only *new* tiles into the master
  python ./scripts/merge_tile_catalogs.py \
    ...
    --publish-parquet --only-new

  # republish one known tile with overwrite (explicit)
  python ./scripts/merge_tile_catalogs.py \
    --tiles-root ./data/tiles/tile-RA123.000-DEC+45.000 \
    --tolerance-arcsec 0.5 --bin-deg 5 \
    --plate-map-csv ./data/metadata/tile_to_dss1red.csv \
    --publish-parquet --overwrite
"""
import argparse
...
def iter_tile_dirs_any(tiles_root: Path):
    ...
    if tiles_root.exists():
        for p in sorted(tiles_root.glob("tile-RA*-DEC*")):
            if p.is_dir():
                yield p

    # sharded sibling (tiles_by_sky next to given root)
    sharded = tiles_root if tiles_root.name == "tiles_by_sky" else (tiles_root.parent / "tiles_by_sky")
    if sharded.exists():
...
def load_plate_map(csv_path: Path) -> dict:
...
def write_partition(root: Path, ra_bin: int, dec_bin: int,
                    df_part: pd.DataFrame, tag: str,
...
def tiles_present_in_master(master_root: Path) -> set:
    """
...
def merge_one_tile(
    ...
    # per-tile parquet (unchanged behaviour)
    ...
    for (rb, db), sub in deduped.groupby(["ra_bin", "dec_bin"], sort=False):
        if sub.empty:
            continue
        write_partition(tile_parquet_root, int(rb), int(db), sub, tile_path.name, overwrite=True)  # local per-tile
        pre_bins.append((int(rb), int(db)))
        count += len(sub)
    ...
    # publish to master (append-only default)
    if publish_root:
        ...
        for (rb, db), sub in deduped.groupby(["ra_bin", "dec_bin"], sort=False):
            if sub.empty:
                continue
            write_partition(pub_root, int(rb), int(db), _enforce_schema(sub.copy()), tile_path.name, overwrite=overwrite)
...
def main():
    ...
    ap.add_argument("--tiles-root", default="./data/tiles", help="Root folder for tiles (flat, sharded, or single tile dir)")
