...
def configure_logger(out_dir: Path) -> logging.Logger:
    ...
    if not lg.handlers:
        h = RotatingFileHandler(
            out_dir / "download.log", maxBytes=512000, backupCount=3
        )
...
# STScI DSS helpers
# -----------------------------------------------------------------------------
def _stscidss_params(
    ra_deg: float, dec_deg: float, size_arcmin: float, survey_key: str, user_agent: str
) -> tuple[str, dict]:
...
def fetch_skyview_dss(
    ...
    *,
    ...
    name = (
        basename
        or f"{tag}_{ra_deg:.6f}_{dec_deg:.6f}_{int(round(size_arcmin))}arcmin.fits"
    )
    out_path = out_dir / name

    url, p = _stscidss_params(ra_deg, dec_deg, size_arcmin, survey, user_agent)
    lg.info(
        "[GET] STScI DSS RA=%.6f Dec=%.6f size=%.2f arcmin v=%s",
        ra_deg,
        dec_deg,
        size_arcmin,
        p.get("v"),
    ...
    if not ok:
        ...
        lg.error(
            "[FAIL] Non-FITS from STScI (Content-Type=%s, len=%d) -> %s",
            ctype,
            len(content),
            bad,
        )
    ...
    if survey.lower() == "poss1-e":
        ...
        if not (
            ...
            lg.error(
                "[ENFORCE] Requested POSS-I E but header shows SURVEY=%r ORIGIN=%r PLATEID=%r",
                sname or "UNKNOWN",
                origin or "UNKNOWN",
                plate or "UNKNOWN",
            )
            ...
            raise RuntimeError(
                f"Non-POSS plate returned by STScI: SURVEY={sname!r} "
                "(declination likely outside POSS coverage)"
            )
...
def tessellate_centers(
    center_ra: float,
    center_dec: float,
    *,
    width_arcmin: float,
    height_arcmin: float,
    tile_radius_arcmin: float = 30.0,
    overlap_arcmin: float = 0.0,
) -> List[Tuple[float, float]]:
    hw = width_arcmin / 2.0
    hh = height_arcmin / 2.0
    r = tile_radius_arcmin
    from math import sqrt, cos, radians

    sy = max(1e-6, sqrt(3.0) * r - overlap_arcmin)
    sx = max(1e-6, 2.0 * r - overlap_arcmin)
    ...
    while off <= hh + 1e-6:
        for sgn in (1.0, -1.0):
            ...
            sxdeg = (sx / 60.0) / cd
            base = center_ra
            raoff = 0.0 if (j % 2) == 0 else 0.5 * sxdeg
            k = 0
            while True:
                ra1 = base + raoff + k * sxdeg
                ra2 = base + raoff - k * sxdeg
                dx1 = abs((ra1 - center_ra) * cd * 60.0)
                dx2 = abs((ra2 - center_ra) * cd * 60.0)
                if dx1 <= hw + 1e-6:
                    res.append((ra1, dec))
                if k > 0 and dx2 <= hw + 1e-6:
                    res.append((ra2, dec))
                if dx1 > hw + 1e-6 and dx2 > hw + 1e-6:
                    break
                k += 1
        j += 1
        off += sy
