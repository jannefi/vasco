...
def _write_overview(
    run_dir: Path, counts: dict, results: list, missing: list[dict] | None = None
) -> None:
    nl = ""
    lines = [
        "# Run Overview",
...
def _ensure_sextractor_csv(tile_dir: Path, pass2_ldac: str | Path) -> Path:
    ...
    try:
        ...
        with fits.open(pass2_ldac, memmap=False) as hdul:
            hdu = next(
                (h for h in hdul if getattr(h, "name", "") == "LDAC_OBJECTS"), None
            )
            if hdu is None:
...
def _post_xmatch_tile(tile_dir, pass2_ldac, *, radius_arcsec: float = 5.0) -> None:
    ...
    try:
        if gaia_csv.exists() and _csv_has_radec(gaia_csv):
            ...
            xmatch_sextractor_with_gaia(
                sex_csv, gaia_csv, out_gaia, radius_arcsec=radius_arcsec
            )
            ...
            print(
                "[POST][WARN]",
                tile_dir.name,
                "Gaia CSV missing or lacks RA/Dec → skipped",
            )
    except Exception as e:
    ...
    try:
        if ps1_csv.exists() and _csv_has_radec(ps1_csv):
            ...
            xmatch_sextractor_with_ps1(
                sex_csv, ps1_csv, out_ps1, radius_arcsec=radius_arcsec
            )
            ...
            print(
                "[POST][WARN]",
                tile_dir.name,
                "PS1 CSV missing or lacks RA/Dec → skipped",
            )
    except Exception as e:
    ...
    try:
        if usnob_csv.exists() and _csv_has_radec(usnob_csv):
            ...
            print(
                "[POST][WARN]",
                tile_dir.name,
                "USNO-B CSV missing or lacks RA/Dec → skipped",
            )
    ...
    if cds_gaia_table:
        ...
        try:
            ...
            print(
                "[POST][WARN]",
                tile_dir.name,
                "STILTS not found; CDS Gaia xmatch skipped",
            )
        except Exception as e:
        ...
        print(
            "[POST][INFO]",
            tile_dir.name,
            "No cds_gaia_table provided; skipping CDS Gaia match",
        )
    # PS1 via CDS
    if cds_ps1_table:
        ...
        try:
            ...
            print(
                "[POST][WARN]",
                tile_dir.name,
                "STILTS not found; CDS PS1 xmatch skipped",
            )
        except Exception as e:
        ...
        print(
            "[POST][INFO]",
            tile_dir.name,
            "No cds_ps1_table provided; skipping CDS PS1 match",
        )
    ...
    for i in range(1, attempts + 1):
        try:
            ...
            if "Too many jobs" in msg or "Service Error" in msg:
                delay += 5.0
            print(
                f"[POST][WARN] cdsskymatch attempt {i}/{attempts} failed: {e}. Retrying in {delay:.1f}s..."
            )
            _t.sleep(delay)
...
def _validate_within_5_arcsec(xmatch_csv: Path) -> Path:
    ...
    if ext_ra is None:
        print(
            "[POST][WARN]",
            tile_dir.name,
            "No external RA/Dec columns found in",
            xmatch_csv.name,
            "— writing empty within5",
        )
        _write_empty()
        return out
    ...
    p1 = subprocess.run(
        ["stilts", "tpipe", f"in={str(xmatch_csv)}", cmd_count, "omode=count"],
        capture_output=True,
        text=True,
    )
    if p1.returncode != 0:
            ...
            missing = [
                {
                    "ra": float(ra),
                    "dec": float(dec),
                    "expected_stem": _expected_stem(
                        ra, dec, args.survey, args.size_arcmin
                    ),
                }
            ]
    ...
    if backend == "local":
        ...
        try:
            if os.getenv("VASCO_DISABLE_USNOB"):
                print(
                    "[POST][INFO]", td.name, "USNO-B disabled by env — skipping fetch"
                )
            else:
                ...
                print(
                    "[POST]",
                    td.name,
                    "USNO-B (VizieR) -> catalogs/usnob_neighbourhood.csv",
                )
        except Exception as e:
...
def cmd_tess(args: argparse.Namespace) -> int:
    ...
    centers = dl.tessellate_centers(
        center_ra,
        center_dec,
        width_arcmin=args.width_arcmin,
        height_arcmin=args.height_arcmin,
        tile_radius_arcmin=args.tile_radius_arcmin,
        overlap_arcmin=args.overlap_arcmin,
    )
    planned = len(centers)
    fits_list = dl.fetch_many(
        centers,
        size_arcmin=args.size_arcmin,
        survey=args.survey,
        pixel_scale_arcsec=args.pixel_scale_arcsec,
    ...
    for fp in fits_list:
        ...
        if backend == "local":
            ...
            try:
                if os.getenv("VASCO_DISABLE_PS1"):
                    print(
                        "[POST][INFO]", td.name, "PS1 disabled by env — skipping fetch"
                    )
                else:
            ...
            try:
                if os.getenv("VASCO_DISABLE_USNOB"):
                    print(
                        "[POST][INFO]",
                        td.name,
                        "USNO-B disabled by env — skipping fetch",
                    )
                else:
            ...
            try:
                _post_xmatch_tile(
                    td, p2, radius_arcsec=float(args.xmatch_radius_arcsec)
                )
            except Exception as e:
                print("[POST][WARN] xmatch failed for", td.name, ":", e)
        elif backend == "cds":
    ...
    for ra, dec in centers:
        ...
        if exp_stem not in processed_stems:
            missing.append(
                {"ra": float(ra), "dec": float(dec), "expected_stem": exp_stem}
            )
...
def cmd_retry_missing(args: argparse.Namespace) -> int:
    ...
    try:
        counts = (
            _read_json(counts_path)
            if counts_path.exists()
            else {"planned": 0, "downloaded": 0, "processed": 0}
        )
    ...
    for rec in missing:
        ...
        for attempt in range(1, args.attempts + 1):
            try:
                ...
                print(
                    f"[WARN] Retry {attempt}/{args.attempts} failed for RA={ra:.6f} Dec={dec:.6f}: {e}"
                )
        ...
        try:
            ...
            export_and_summarize(
                p2, td, export=args.export, histogram_col=args.hist_col
            )
            radius_arcmin = args.size_arcmin * (2**0.5) * 0.5
            try:
                parts = stem.split("_")
                ...
                try:
                    if os.getenv("VASCO_DISABLE_PS1"):
                        print(
                            "[POST][INFO]",
                            td.name,
                            "PS1 disabled by env — skipping fetch",
                        )
                    else:
                        fetch_ps1_neighbourhood(td, ra_t, dec_t, radius_arcmin)
                except Exception as e:
            ...
            print("[ERROR] Tools missing for", stem, ":", e)
            still_missing.append(rec)
        except Exception as e:
            print("[ERROR] Processing failed for", stem, ":", e)
            still_missing.append(rec)
...
def main(argv: List[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        ...
        description="VASCO pipeline orchestrator (download + 2-pass + export + QA + xmatch: local/CDS)",
    )
    sub = p.add_subparsers(dest="cmd")
    one = sub.add_parser(
        "one2pass",
        help="One RA/Dec -> download -> two-pass pipeline (post-xmatch backend selectable)",
    )
    one.add_argument(
        "--ra",
        type=str,
        required=True,
        help="RA in decimal deg or sexagesimal hh:mm:ss.s",
    )
    one.add_argument(
        "--dec",
        type=str,
        required=True,
        help="Dec in decimal deg or sexagesimal ±dd:mm:ss.s",
    )
    ...
    one.add_argument(
        ...
        choices=["local", "cds"],
        default="local",
        help="Choose local (tskymatch2) or CDS (cdsskymatch) backend",
    )
    one.add_argument(
    ...
    one.add_argument(
        "--cds-gaia-table",
        default=os.getenv("VASCO_CDS_GAIA_TABLE"),
        help="VizieR table ID for Gaia (CDS backend). Env VASCO_CDS_GAIA_TABLE respected.",
    )
    one.add_argument(
        "--cds-ps1-table",
        default=os.getenv("VASCO_CDS_PS1_TABLE"),
        help="VizieR table ID for PS1 (CDS backend). Env VASCO_CDS_PS1_TABLE respected.",
    )
    one.set_defaults(func=cmd_one)

    tess = sub.add_parser(
        "tess2pass",
        help="Tessellate region and run two-pass pipeline per tile (post-xmatch backend selectable)",
    )
    tess.add_argument(
        "--center-ra",
        type=str,
        required=True,
        help="Center RA in decimal deg or sexagesimal hh:mm:ss.s",
    )
    tess.add_argument(
        "--center-dec",
        type=str,
        required=True,
        help="Center Dec in decimal deg or sexagesimal ±dd:mm:ss.s",
    )
    ...
    rt = sub.add_parser(
        "retry-missing",
        help="Retry tiles for an existing run (post-xmatch backend selectable)",
    )
    rt.add_argument(
        "run_dir", help="Existing run directory (data/runs/run-YYYYMMDD_HHMMSS)"
    )
