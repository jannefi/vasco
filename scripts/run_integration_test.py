...
def main(argv=None):
    ...
    p.add_argument('--prefer-stsci', action='store_true', help='Log preference for STScI DSS (and warn on SkyView).')
    p.add_argument('--prefer', choices=['stsci','skyview'], help='Explicitly log preference for a given service.')

    # Bright star mask
    p.add_argument('--mask-usno', action='store_true', help='Call USNO-B1 placeholder mask.')
    ...
    if args.mask_usno:
        ...
        mask_bright_stars_usno_b1(args.usno, args.ra, args.dec, radius_deg=args.radius_deg)
    ...
    if args.xmatch:
        ...
        xmatch_with_stilts(args.t1, args.t2, args.out, join_type=args.join, matcher=args.matcher, radius_arcsec=args.radius)
