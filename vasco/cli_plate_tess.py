...
def _sample_positions(
    positions: List[Tuple[float, float]], fraction: float, seed: int = 42
) -> List[Tuple[float, float]]:
    if fraction >= 0.999:
        return positions
...
def build_tiles(
    plates_json: Path, out_csv: Path, *, default_fraction=0.2, seed: int = 42
) -> int:
    pdata = json.loads(Path(plates_json).read_text(encoding="utf-8"))
    rows: List[Dict[str, Any]] = []
    for p in pdata:
        plate_id = p.get("plate_id", "unknown")
        ra_val = _first_present(
            p, ["center_ra_deg", "center_ra", "ra", "center_ra_hms"]
        )
        dec_val = _first_present(
            p, ["center_dec_deg", "center_dec", "dec", "center_dec_dms"]
        )
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(
        prog="vasco.cli_plate_tess",
        description="Plate-aware tessellation (sample or full cover) for DSS/POSS cutouts; accepts sexagesimal or decimal plate centers.",
    )
    ...
    if args.cmd == "build":
        n = build_tiles(
            Path(args.plates_json),
            Path(args.out_csv),
            default_fraction=float(args.default_fraction),
            seed=int(args.seed),
        )
