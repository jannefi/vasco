...
def _latest_run() -> Path:
    runs = Path("data/runs")
    cands = sorted(
        (d for d in runs.glob("run-*") if d.is_dir()),
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="STILTS post-xmatch for latest/selected run"
    )
    ap.add_argument(
        "--run",
        type=str,
        default=None,
        help="Run directory (default: latest under data/runs)",
    )
    ap.add_argument(
        "--radius",
        type=float,
    ...
    ap.add_argument(
        ...
        type=str,
        default="1and2",
        help="STILTS join type: 1and2|1or2|... (default: 1and2)",
    )
    ...
    xmatch_sextractor_with_gaia(
        sex_csv, gaia_csv, x_gaia, radius_arcsec=args.radius, join_type=args.join
    )
    xmatch_sextractor_with_ps1(
        sex_csv, ps1_csv, x_ps1, radius_arcsec=args.radius, join_type=args.join
    )
