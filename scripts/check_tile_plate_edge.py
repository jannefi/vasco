...
# -*- coding: utf-8 -*-
"""
check_tile_plate_edge.py (v4.3 — DSS plate geometry without WCS + orientation enforcement)

What's new in v4.3
------------------
• Keeps v4.2 robust plate resolver (handles sidecars that point to *_30arcmin.fits).
• Adds automatic orientation enforcement so overlays match Aladin/DSS display
  conventions (East-left, North-up). We auto-detect handedness using a tiny
  (+RA, +Dec) test at the plate center and flip axes if needed.
• Retains robust JSON loader, sharded overlays, and classification logic.

Limitations
-----------
• Still an approximation of plate optics (no AMDX/AMDY polynomials). For
  “edge proximity” this is fine; for sub-arcsecond placement at the rim,
  a full plate model would be required.
"""
...
# Discovery accepts both flat and sharded layouts, hyphen/underscore variants
...
def resolve_plate_json_from_fits_field(tile_dir: Path, dss_root_candidates: list[Path]):
    raw = tile_dir / 'raw'
    ...
    for root in dss_root_candidates:
    ...
    for hn in header_names:
        ...
        if cand2.exists():
            return cand2, ''
    return None, f'not found: tried {", ".join(header_names)} under ' + ";".join(str(r) for r in dss_root_candidates) + ' or raw/'
...
def load_plate_core_from_json(path: Path):
    ...
    return {
        ...
        'cx': cx,
        'cy': cy,
        'as_per_px_x': as_per_px_x,
        'as_per_px_y': as_per_px_y,
    }, ''
...
def radec_to_plate_pixels_gnomonic(ra_deg: np.ndarray, dec_deg: np.ndarray, plate: dict) -> np.ndarray:
    ...
    xi_as = xi * RAD2AS
    eta_as = eta * RAD2AS
    x = cx + (xi_as / as_px_x)   # note: orientation will be enforced below
    y = cy + (eta_as / as_px_y)
...
def enforce_east_left_orientation(plate: dict):
    ...
    East test: x should DECREASE when RA increases slightly at plate center.
    North test: y should INCREASE when Dec increases slightly.
    """
    eps_arcsec = 5.0
...
def render_overlay_png(out_png: Path, plate_nx: int, plate_ny: int, tile_poly_xy: np.ndarray,
    ...
    if subtitle:
        ax.text(0.02, 0.98, subtitle, transform=ax.transAxes, va='top', ha='left', fontsize=9, color='gray')
    ...
    ax.set_xlim(0, plate_nx+1)
    ax.set_ylim(0, plate_ny+1)
...
def main():
    ap = argparse.ArgumentParser(description='Tile vs Plate Edge (v4.3): DSS mapping + orientation enforcement (no plate WCS)')
    ...
    for td in iter_tile_dirs(tiles_root):
        ...
        # Plate info (no WCS) — robust resolver
        plate_json, plate_err = resolve_plate_json_from_fits_field(td, dss_roots)
        if plate_json is None:
            rows_out.append({'tile_id': tile_id, 'plate_filename': '', 'plate_nx': '', 'plate_ny': '',
                             ...
                             'class_px': 'off_plate', 'class_arcsec': 'off_plate', 'notes': plate_err})
            continue

        plate_info, core_err = load_plate_core_from_json(plate_json)
        if core_err:
            rows_out.append({'tile_id': tile_id, 'plate_filename': plate_json.name,
                             'tile_center_ra_deg': f"{tile_cent[0]:.6f}" if tile_cent else '',
                             'tile_center_dec_deg': f"{tile_cent[1]:.6f}" if tile_cent else '',
                             'plate_center_ra_deg': '', 'plate_center_dec_deg': '', 'sep_arcmin': '',
                             'plate_nx': '', 'plate_ny': '',
                             'min_edge_dist_px': '', 'min_edge_dist_arcsec': '',
                             'class_px': 'off_plate', 'class_arcsec': 'off_plate', 'notes': core_err})
            continue

        orient = enforce_east_left_orientation(plate_info)

        # tile center by WCS if not in name
        ...
        if tile_wcs is None or not tnx or not tny:
            ...
            rows_out.append({'tile_id': tile_id, 'plate_filename': plate_json.name,
                             ...
                             'min_edge_dist_px': '', 'min_edge_dist_arcsec': '',
                             'class_px': 'off_plate', 'class_arcsec': 'off_plate', 'notes': note})
            continue
        ...
        # orientation enforcement for classification points
        if orient['flip_x']:
        ...
        as_per_px_mean = 0.5*(plate_info['as_per_px_x'] + plate_info['as_per_px_y'])
        as_margin = px_margin * as_per_px_mean if not math.isnan(px_margin) else float('nan')

        # classify
        def classify_px(min_dist_px: float, plate_nx: int, plate_ny: int, th_px: float, th_frac: float) -> str:
            if math.isnan(min_dist_px): return 'off_plate'
            if min_dist_px < 0: return 'edge_touch'
            thresh = max(float(th_px), float(th_frac) * float(min(plate_nx, plate_ny)))
            return 'core' if min_dist_px >= thresh else 'near_edge'

        def classify_arcsec(min_dist_arcsec: float, th_arcsec: float) -> str:
            if min_dist_arcsec is None or math.isnan(min_dist_arcsec): return 'off_plate'
            if min_dist_arcsec < 0: return 'edge_touch'
            return 'core' if min_dist_arcsec >= th_arcsec else 'near_edge'
        ...
        if args.plot and (args.plot_all or c_px in ('near_edge','edge_touch') or c_as in ('near_edge','edge_touch')):
            # polygon from the first 5 points (close the loop)
            poly_world = world[:5,:]
            poly_plate = radec_to_plate_pixels_gnomonic(poly_world[:,0], poly_world[:,1], plate_info)
            if orient['flip_x']:
                poly_plate[:,0] = 2.0*plate_info['cx'] - poly_plate[:,0]
            if orient['flip_y']:
                poly_plate[:,1] = 2.0*plate_info['cy'] - poly_plate[:,1]
            ...
            subtitle = f"plate: {plate_json.name} | min_edge: {px_margin:.1f}px / {as_margin:.1f}\" | sep: {sep_arcmin:.2f}' | orient: x={'L' if orient['flip_x'] else 'R'} y={'U' if not orient['flip_y'] else 'D'}"
            render_overlay_png(out_png, plate_info['nax1'], plate_info['nax2'], poly_plate,
                               title, subtitle, px_margin if not math.isnan(px_margin) else float('nan'),
                               as_margin if not math.isnan(as_margin) else float('nan'))

    out_csv.parent.mkdir(parents=True, exist_ok=True)
    with out_csv.open('w', newline='', encoding='utf-8') as f:
        fieldnames = ['tile_id','plate_filename','plate_nx','plate_ny',
