...
#!/usr/bin/env python3
"""
Generate per-tile 'unmatched' CSVs for both flattened and legacy layouts.

Layouts supported:
  - Flattened: <DATA>/tiles/<tile_id>/{catalogs,xmatch}/...
  - Legacy:    <DATA>/runs/run-*/tiles/<tile_id>/{catalogs,xmatch}/...
...
def run_stilts_unmatched(sex_csv: Path, other_csv: Path, out_csv: Path,
                         ra1: str, dec1: str, ra2: str, dec2: str,
                         radius_arcsec: float, join: str = '1not2', dry_run: bool = False):
...
def unmatched_from_cdss(sex_csv: Path, cdss_xmatch_csv: Path, out_csv: Path,
                        tol_arcsec: float, dry_run: bool = False):
    ...
    return run_stilts_unmatched(
        sex_csv, cdss_xmatch_csv, out_csv,
...
def process_tile(tile_dir: Path, tol_local: float, tol_cdss: float,
                 backend_mode: str, dry_run: bool) -> int:
    """
    ...
    # Triggers (presence heuristics): xmatch/sex_*_xmatch.csv
    if backend_mode in ('auto', 'local', 'both'):
        # GAIA local
        if (xmatch / 'sex_gaia_xmatch.csv').exists():
            gaia_loc = catalogs / 'gaia_neighbourhood.csv'
            out_csv  = xmatch / 'sex_gaia_unmatched.csv'
            ra2, dec2 = detect_radec_columns(gaia_loc, [
                ('RA_ICRS', 'DE_ICRS'),
                ('RAJ2000', 'DEJ2000'),
                ('ra', 'dec'), ('RA', 'DEC')
            ])
            if ra2 and dec2:
                if run_stilts_unmatched(sex_csv, gaia_loc, out_csv,
                                        'ALPHA_J2000', 'DELTA_J2000',
                                        ra2, dec2, tol_local, dry_run=dry_run):
                    wrote += 1

        # PS1 local
        if (xmatch / 'sex_ps1_xmatch.csv').exists():
            ps1_loc = catalogs / 'ps1_neighbourhood.csv'
            out_csv = xmatch / 'sex_ps1_unmatched.csv'
            if run_stilts_unmatched(sex_csv, ps1_loc, out_csv,
                                    'ALPHA_J2000', 'DELTA_J2000',
                                    'raMean', 'decMean', tol_local, dry_run=dry_run):
                wrote += 1

        # USNOB local
        if (xmatch / 'sex_usnob_xmatch.csv').exists():
            usnob_loc = catalogs / 'usnob_neighbourhood.csv'
            out_csv   = xmatch / 'sex_usnob_unmatched.csv'
            if run_stilts_unmatched(sex_csv, usnob_loc, out_csv,
                                    'ALPHA_J2000', 'DELTA_J2000',
                                    'RAJ2000', 'DEJ2000', tol_local, dry_run=dry_run):
                wrote += 1

    # ---------- CDS backend ----------
    # Inputs (already xmatched via CDS): xmatch/sex_gaia_xmatch_cdss.csv, sex_ps1_xmatch_cdss.csv
    if backend_mode in ('auto', 'cds', 'both'):
        # GAIA CDS
        gaia_cdss = xmatch / 'sex_gaia_xmatch_cdss.csv'
        if gaia_cdss.exists():
        ...
        # PS1 CDS
        ps1_cdss = xmatch / 'sex_ps1_xmatch_cdss.csv'
        if ps1_cdss.exists():
...
def find_tile_dirs(data_dir: Path):
    """Yield tile directories for flattened or legacy layout."""
    flattened = data_dir / 'tiles'
    if flattened.exists():
        # <data>/tiles/<tile_id>/{catalogs,xmatch}
        for p in sorted(flattened.glob('*/')):
            yield p
        return

    legacy_root = data_dir / 'runs'
    if legacy_root.exists():
        # <data>/runs/run-*/tiles/<tile_id>/{catalogs,xmatch}
...
def main():
    ap = argparse.ArgumentParser(
        description='Generate per-tile unmatched CSVs for flattened or legacy VASCO layouts.'
    )
    ap.add_argument('--data-dir', default='./data',
                    help='Path to data directory that contains tiles/ or runs/')
    ...
    if not tiles:
        print('[ERROR] Could not find tiles under:', data_dir / 'tiles', 'or', data_dir / 'runs')
        print('        Expected <data>/tiles/<tile>/... or <data>/runs/run-*/tiles/<tile>/...')
        return 2
