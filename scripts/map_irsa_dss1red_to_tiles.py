...
# -*- coding: utf-8 -*-
"""
Map VASCO tiles to IRSA DSS1-red plates using local headers.
Layout-aware tile discovery.
"""
import argparse
import csv
import glob
...
def main():
    ...
    ap.add_argument('--max-positional-sep-deg', type=float, default=4.0,
                    help='Reject positional fallback with separation >= cap')
    args = ap.parse_args()
    ...
    if args.irsa_json_dir:
        ...
        # FITS fallback omitted here for brevity
        irsa_rows = []
    else:
    ...
    for tile_dir in iter_tile_dirs_any(Path(args.tiles_dir)):
        ...
        if t_region and t_region in region_idx:
            ...
            if t_region and args.no_positional_when_region_present:
                unresolved = True; unresolved_reason = 'region_missing_in_irsa_index'
            else:
