...
class BrightStar:
    ra: float
    dec: float
    rmag: float  # proxy magnitude (PS1 rMeanPSFMag)
...
def fetch_bright_ps1(
    ...
    within the given radius. Default radius 90' mirrors the USNO-B1.0
    search radius used in the paper; reduce to 35' to match earlier behavior.
    """
...
class SpikeRuleLine:
    ...
    # Paper form: Rmag <= -0.09 * d_arcsec + 15.3 ; equality is treated as KEEP to avoid over-rejecting
...
def apply_spike_cuts(
    ...
    Reject if nearest bright star satisfies:
    - m_near <= 12.4                        (const rule)
    - m_near <  a * d_arcsec + b            (line rule; strict inequality)
    """
    ...
    for r in tile_rows:
        ...
        dmin_arcsec = float('inf'); m_near: Optional[float] = None
        for b in bright:
            da = angsep_arcsec(ra, dec, b.ra, b.dec)
            if da < dmin_arcsec:
                dmin_arcsec = da; m_near = b.rmag
