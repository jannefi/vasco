#!/usr/bin/env bash
# make_masked_union_duckdb.sh — v3.3 (tile-safe version)
# Fix: tile_id is now ALWAYS extracted from row_id. No majority-tile logic.
...
-- Survivors: NUMBER and tile_id from survivors parquet
...
-- Base: extract tile_id from row_id ONLY
CREATE OR REPLACE VIEW base AS
SELECT
    m.row_id,
    m.NUMBER,
    SPLIT_PART(m.row_id, ':', 1) AS tile_id
FROM chunk_map m;
...
-- Flags (vosa, scos, ptf, vsx, skybot)
