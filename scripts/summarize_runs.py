...
def summarize_run(run_dir: str) -> dict:
    ...
    gaia_match = glob_sum(
        str(tiles / "*" / "xmatch" / "sex_gaia_xmatch_cdss_within5arcsec.csv")
    )
    if gaia_match == 0:
        gaia_match = glob_sum(str(tiles / "*" / "xmatch" / "sex_gaia_xmatch.csv"))
    ps1_match = glob_sum(
        str(tiles / "*" / "xmatch" / "sex_ps1_xmatch_cdss_within5arcsec.csv")
    )
    if ps1_match == 0:
        ps1_match = glob_sum(str(tiles / "*" / "xmatch" / "sex_ps1_xmatch.csv"))
    # Unmatched (CDS vs local)
    gaia_unmatch_cdss = glob_sum(
        str(tiles / "*" / "xmatch" / "sex_gaia_unmatched_cdss.csv")
    )
    ps1_unmatch_cdss = glob_sum(
        str(tiles / "*" / "xmatch" / "sex_ps1_unmatched_cdss.csv")
    )
    gaia_unmatch_local = glob_sum(
        str(tiles / "*" / "xmatch" / "sex_gaia_unmatched.csv")
    )
...
def write_cross_run_md(data_dir: str, rows: list[dict]) -> str:
    ...
    # Table 1: counts
    lines.append(
        '| run | planned | downloaded | processed | detections | GAIA≤5" | PS1≤5" | GAIA unmatched (CDS) | PS1 unmatched (CDS) | GAIA unmatched (local) | PS1 unmatched (local) | USNOB unmatched |'
    )
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for r in rows:
        lines.append(
            "| {run} | {planned} | {downloaded} | {processed} | {detections} | {gaia_matched} | {ps1_matched} | {gaia_unmatched_cdss} | {ps1_unmatched_cdss} | {gaia_unmatched_local} | {ps1_unmatched_local} | {usnob_unmatched} |".format(
                **r
            )
        )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "| run | GAIA matched % | PS1 matched % | GAIA unmatched (CDS) % | PS1 unmatched (CDS) % | GAIA unmatched (local) % | PS1 unmatched (local) % | USNOB unmatched % |"
    )
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
    for r in rows:
        lines.append(
            "| {run} | {gaia_matched_pct:.2f} | {ps1_matched_pct:.2f} | {gaia_unmatched_cdss_pct:.2f} | {ps1_unmatched_cdss_pct:.2f} | {gaia_unmatched_local_pct:.2f} | {ps1_unmatched_local_pct:.2f} | {usnob_unmatched_pct:.2f} |".format(
                **r
            )
        )
    lines.append("")
...
def write_per_run_md(run_dir: str, r: dict) -> str:
    ...
    lines.append("")
    lines.append(
        '| catalog | matched (≤5") | unmatched (CDS) | unmatched (local) | matched % | unmatched (CDS) % | unmatched (local) % |'
    )
    lines.append("|---|---:|---:|---:|---:|---:|---:|")
    lines.append(
        "| GAIA | {gaia_matched} | {gaia_unmatched_cdss} | {gaia_unmatched_local} | {gaia_matched_pct:.2f} | {gaia_unmatched_cdss_pct:.2f} | {gaia_unmatched_local_pct:.2f} |".format(
            **r
        )
    )
    lines.append(
        "| PS1  | {ps1_matched} | {ps1_unmatched_cdss} | {ps1_unmatched_local} | {ps1_matched_pct:.2f} | {ps1_unmatched_cdss_pct:.2f} | {ps1_unmatched_local_pct:.2f} |".format(
            **r
        )
    )
    lines.append(
        "| USNOB (unmatched only) | — | — | {usnob_unmatched} | — | — | {usnob_unmatched_pct:.2f} |".format(
            **r
        )
    )
    lines.append("")
...
def main():
    ap = argparse.ArgumentParser(
        description="Summarize VASCO runs (matched/unmatched) to Markdown and CSV (VS Code-friendly formatting)."
    )
