...
Layout-aware: scans tiles under ./data/tiles and ./data/tiles_by_sky.
"""
import os
import glob
import argparse
...
def iter_tile_dirs_any(root: str):
...
def process_tile(tile_dir, args):
    ...
    gdf = apply_quality_mask(gdf, mag_auto_threshold=args.mag_auto_threshold, elongation_max=args.elongation_max)
...
def main():
    ap = argparse.ArgumentParser(description="Per-tile Gaia-tied plate solution for VASCO (layout-aware).")
    ...
    args = ap.parse_args()
    tiles = list(iter_tile_dirs_any(args.tiles_folder))
