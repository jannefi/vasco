...
## Adding plate id to master optical Parquet

Change of record: We standardize on plate_region (e.g., XE325) as the only canonical plate identifier for DSS1-red. REGION is unique per plate (~937 plates) and directly locates the header JSON. PLTLABEL/PLATEID are not stored in master outputs.
Scope: master optical Parquet first (then inherited by IR-aware/derived datasets).

Prereqs

- Public helpers (in repo): ./scripts/map_irsa_dss1red_to_tiles.py, ./scripts/write_dss1red_title_per_tile.py
- ./data/metadata/tile_to_dss1red.csv must contain at least: tile_id, irsa_region (they are autogenerated if you have used the correct scripts)
- Full plate FITS not required locally if header JSONs are present; FITS are large and downloaded by users from public archives referenced in README.
- DuckDB command line CLI installed

### Step 0 — Refresh the mapping (region-only)
```bash
python ./scripts/map_irsa_dss1red_to_tiles.py \
  --tiles-dir ./data/tiles_by_sky \
  --irsa-json-dir ./data/dss1red-headers \
  --irsa-index ./data/metadata/irsa_dss1red_index.csv \ # note: does not exist automatically
  --out-dir ./data/metadata
```
### Step 1 — Coverage check 

Note: adjust paths and filenames if needed. Run with duckdb CLI.

```bash
duckdb -c "INSTALL parquet; LOAD parquet;
WITH core AS (
  SELECT DISTINCT tile_id
  FROM read_parquet('./data/local-cats/_master_optical_parquet/ra_bin=*/dec_bin=*/*.parquet')
), map AS (
  SELECT DISTINCT tile_id
  FROM read_csv_auto('./data/metadata/tile_to_dss1red.csv')
)
SELECT COUNT(*) AS core_tiles FROM core;  -- expect ~tile count
SELECT COUNT(*) AS map_tiles  FROM map;
SELECT c.tile_id
FROM core c LEFT JOIN map m USING (tile_id)
WHERE m.tile_id IS NULL
ORDER BY c.tile_id LIMIT 50;"
```

###  Step 2 — Partition-wise writer 

Note: adjust filenames/paths if needed
```bash
MASTER="./data/local-cats/_master_optical_parquet"
MAPCSV="./data/metadata/tile_to_dss1red.csv"     # must contain: tile_id, irsa_region
OUT="./data/local-cats/_master_optical_parquet_with_plateid_region"
TMP="/tmp/vasco_duckdb_tmp"                       # portable default; override if needed

mkdir -p "$TMP" "$OUT"

# Walk each (ra_bin, dec_bin) partition of the *base* master
find "$MASTER" -type d -path "$MASTER/ra_bin=*/dec_bin=*" | while read -r PARTDIR; do
  ...
  duckdb -c "
    ...
    -- read this partition of the base master (writer-agnostic glob)
...
duckdb -c "
  ...
  SELECT * FROM parquet_scan('${OUT}/ra_bin=*/dec_bin=*/*.parquet');
  DESCRIBE SELECT * FROM cand LIMIT 0;              -- should list plate_id
  SELECT COUNT(*) AS new_rows FROM cand;
  SELECT SUM(CASE WHEN plate_id IS NULL OR plate_id='' THEN 1 ELSE 0 END) AS null_plate_id FROM cand;
  SELECT plate_id, COUNT(*) AS n FROM cand GROUP BY 1 ORDER BY n DESC LIMIT 10;
"
# safe "promote"
ln -sfn _master_optical_parquet_with_plateid_region ./data/local-cats/_master_optical_parquet_CURRENT
```

### step 4 helper script in repository
bash script ./scripts/enrich_master_with_plate_region.sh creates the Parquet with plate_id(region)
