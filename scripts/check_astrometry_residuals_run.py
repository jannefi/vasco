#!/usr/bin/env python3
"""
astrometry_residuals_corrected.py (layout-aware)
Checks per-tile residuals using final_catalog_wcsfix.csv and Gaia within5 xmatch.
"""
import os
import glob
import pandas as pd
...
def iter_tile_dirs_any(base):
    flat = os.path.join(base, "tile-*")
    for p in sorted(glob.glob(flat)):
        if os.path.isdir(p): yield p
    sharded = os.path.join(os.path.dirname(base), "tiles_by_sky", "ra_bin=*","dec_bin=*","tile-*")
    for p in sorted(glob.glob(sharded)):
        if os.path.isdir(p): yield p

def main():
    ap = argparse.ArgumentParser(description="Per-tile astrometry residuals checker (Gaia-tied, layout-aware).")
    ap.add_argument("--tiles-folder", default="./data/tiles", help="Root folder of tiles (default: ./data/tiles)")
    args = ap.parse_args()
    base = args.tiles_folder
    tiles = list(iter_tile_dirs_any(base))
    out_path = os.path.join(os.path.dirname(base), "astrometry_summary.csv")
    ...
    for t in tiles:
        ...
        try:
            ...
            continue
        # pick detection RA/Dec columns in Gaia xmatch
