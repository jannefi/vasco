...
# Default: sequential (THREADS=1). You can raise gently later.
# Produces:
#   parts/flags_vsx__chunk_*.parquet
#   canonical/flags_vsx_known_variables.parquet
#   audit/flags_vsx_parts_all.parquet
set -euo pipefail
...
THREADS="${THREADS:-1}"                      # conservative start
...
# bounded parallelism (sequential by default)
pids=(); active=0
...
echo "[paths] CANON=${CANON}"
echo "[paths] AUDIT=${AUDIT}"
