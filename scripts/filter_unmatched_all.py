#!/usr/bin/env python3
# FAST v3 unmatched/union generator — layout-aware and with a fixed positional fallback call
...
def stilts_positional_unmatched(det_csv: Path, other_csv: Path, out_csv: Path,
    ...
    """Find detections in det_csv (ra1/dec1) that lack positional matches in other_csv (ra2/dec2)."""
...
# ----- catalog picker -----
def pick_detection_catalog(tile_dir: Path):
    import pandas as pd
...
def process_tile(tile_dir: Path, tol_cdss: float, positional: bool, dry: bool) -> int:
    ...
    if target_union.exists() and stilts_exact_id_antijoin(det_csv, target_union, xmatch/'no_optical_counterparts.csv', dry): wrote += 1

    if positional:
        if gaia_match and stilts_positional_unmatched(det_csv, gaia_match, xmatch/'sex_gaia_unmatched_cdss_pos.csv',
                                                     ra1_col, dec1_col, 'RAJ2000','DEJ2000', tol_cdss, dry): wrote += 1
        if ps1_match and stilts_positional_unmatched(det_csv, ps1_match, xmatch/'sex_ps1_unmatched_cdss_pos.csv',
                                                    ra1_col, dec1_col, 'RAJ2000','DEJ2000', tol_cdss, dry): wrote += 1
    return wrote

# ---- CLI ----
def main():
    ap = argparse.ArgumentParser(description='FAST v3 unmatched/union (layout-aware).')
    ...
    total = 0
    for tile in iter_tile_dirs_any(Path(args.data_dir)):
